...
class RLOOTrainer(_BaseTrainer):
    """
    Trainer for REINFORCE Leave-One-Out (RLOO).

    RLOO uses a leave-one-out baseline estimator for variance reduction. For each completion,
    the baseline is the average reward of all other completions in the same group, providing
    an unbiased and low-variance gradient estimate.
    ...
    ```python
    >>> trainer = RLOOTrainer(model=model, reward_functions=[reward_fn])
        ...
        for prompt in prompts:
            ...
            if self._vllm_available:
                ...
                for output in outputs:
                    # RLOO discards logprobs from vLLM outputs
    ...
    def train(self, prompts: list[str]):
        ...
        for epoch in range(self.num_train_epochs):
            ...
            # Step 4: Compute leave-one-out advantages (RLOO-specific)
    ...
    def _compute_rloo_advantages(self, rewards):
        """
        Compute RLOO leave-one-out advantages.

        For each completion, the advantage is the reward minus the mean reward of all other
        completions in the same group.
        ...
        for i in range(0, len(rewards), self.num_generations):
            ...
            for j, reward in enumerate(group):
                # Leave-one-out baseline: mean of all other rewards in the group
                baseline = (group_sum - reward) / (len(group) - 1) if len(group) > 1 else 0.0
                advantages.append(reward - baseline)
        return advantages

    def _compute_loss(self, inputs, advantages):
        """Compute the RLOO policy loss (placeholder)."""
